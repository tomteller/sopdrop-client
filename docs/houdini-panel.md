...
- `sopdrop-houdini/scripts/python/nodegraphhooks.py` — Houdini's network editor imports the first `nodegraphhooks` module on the Python path and calls `createEventHandler(uievent, pending_actions)` per UI event. On a `keyhit` of `Shift+Tab` (or `Shift+Backtab`/`Backtab` on some Qt platforms, matched case-insensitively) it calls `sopdrop.tabmenu.show_tab_menu(editor=uievent.editor)` and returns `(None, True)` to swallow the event. Everything else returns `(None, False)`. **Placement matters**: `import nodegraphhooks` resolves against sys.path, which includes each HOUDINI_PATH entry's `pythonX.Ylibs` but not necessarily `scripts/python`. So `python3.{9,10,11,12,13,14}libs/nodegraphhooks.py` are loader shims that `runpy.run_path()` the canonical file above, located relative to their own `__file__` — one directory per Houdini Python version (H19.5=3.9, H20.0=3.10, H20.5/21=3.11, **H22=3.13**), with 3.12/3.14 as headroom. Missing the directory for a Houdini version means Shift+Tab silently does nothing there, which is exactly how it broke on H22. Supporting a new version is a copy of the shim; the handler exists once. Studios that already ship their own `nodegraphhooks.py` must merge the handler (Houdini imports only one).
