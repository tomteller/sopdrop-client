...
class AssetCardWidget(QtWidgets.QFrame):
    ...
    def _setup_ui(self):
        ...
        if sync_status in ('synced', 'syncing', 'modified'):
            ...
            if sync_status == 'synced':
                ...
                sync_icon.setToolTip("Modified locally")
    ...
    def contextMenuEvent(self, event):
        ...
        # Different actions based on asset type
        ...
        # Collections submenu (with nested children)
        coll_menu = menu.addMenu("\u25A3  Add to Collection")
        if SOPDROP_AVAILABLE:
            tree = library.get_collection_tree()
            current = set(c['id'] for c in self.asset.get('collections', []))
            self._build_collection_submenu(coll_menu, tree, current)
            if tree:
        ...
        elif self.asset.get('sync_status', 'local_only') == 'local_only':
        ...
        if SOPDROP_AVAILABLE:
            ...
            if other_lib:
                ...
                menu.addAction(f"\u2295  Copy to {lib_name}").triggered.connect(
                    lambda checked=False, t=other_lib: self._copy_to_library(t)
                )
                menu.addAction(f"\u2192  Move to {lib_name}").triggered.connect(
                    lambda checked=False, t=other_lib: self._move_to_library(t)
                )

        menu.addSeparator()
        menu.addAction("\u2715  Delete").triggered.connect(lambda: self.delete_requested.emit(self.asset['id']))
...
class LibraryPanel(QtWidgets.QWidget):
    ...
    def _refresh_assets(self):
        ...
        # Hide pending-delete assets
        pending_id = getattr(self, '_pending_delete_id', None)
        if pending_id:
            assets = [a for a in assets if a.get('id') != pending_id]
    ...
    def _sync_from_cloud(self):
        ...
        try:
            if is_team_mode:
                cloud_assets = library.get_team_saved_assets(team_slug)
                source_name = f"team '{team_slug}'"
            else:
                cloud_assets = library.get_cloud_saved_assets()
                source_name = "your cloud library"

            if not cloud_assets:
                if is_team_mode:
                    hou.ui.displayMessage(
                        f"No saved assets found in team '{team_slug}'.\n\n"
                        "Add assets to your team library on sopdrop.com first!",
                        title="Team Sync"
                    )
                else:
                    hou.ui.displayMessage(
                        "No saved assets found in your cloud library.\n\n"
                        "Save assets on sopdrop.com first!",
                        title="Cloud Sync"
                    )
                return

            title = "Team Sync" if is_team_mode else "Cloud Sync"
            reply = QtWidgets.QMessageBox.question(
                self, title,
                f"Found {len(cloud_assets)} saved assets in {source_name}.\n\n"
                "Download them to your local library?"
            )
            if reply != QtWidgets.QMessageBox.Yes:
                return

        except Exception as e:
            hou.ui.displayMessage(f"Failed to check cloud library: {e}")
            return

        try:
            # Sync saved assets (each cloud folder becomes a local collection)
            if is_team_mode:
            ...
            if result.get('error'):
                ...
                msg = f"Synced {result['synced']} assets"
                if result['skipped']:
...
class HoudiniIconBrowser(QtWidgets.QDialog):
    ...
    # Common icon categories with representative icons
    ICON_CATEGORIES = {
    ...
    def _setup_ui(self):
        self.setWindowTitle("Select Houdini Icon")
        ...
        search_label.setStyleSheet(f"color: {COLORS['text']}; font-size: 11px;")
        search_row.addWidget(search_label)

        self.search_input = QtWidgets.QLineEdit()
        self.search_input.setPlaceholderText("Type to search icons (e.g., scatter, copy, transform)")
        ...
        for category, icons in self.ICON_CATEGORIES.items():
            ...
            self._icon_grids[category] = (container, grid, icons)
        ...
        preview_label = QtWidgets.QLabel("Selected:")
        preview_label.setStyleSheet(f"color: {COLORS['text_dim']}; font-size: 11px;")
        ...
        self.preview_name = QtWidgets.QLabel("None")
        self.preview_name.setStyleSheet(f"color: {COLORS['text']}; font-size: 11px;")
        preview_row.addWidget(self.preview_name)
        ...
        select_btn = QtWidgets.QPushButton("Select Icon")
    ...
    def _select_icon(self, icon_name):
        ...
        try:
            ...
            if icon and not icon.isNull():
                ...
                self.preview_btn.setText(icon_name[:2])
        except:
            self.preview_btn.setText(icon_name[:2])
        ...
        for cat, (container, grid, icons) in self._icon_grids.items():
            for i in range(grid.count()):
                item = grid.itemAt(i)
                if item and item.widget():
                    widget = item.widget()
                    is_sel = widget.property("icon_name") == icon_name
                    widget.setProperty("is_selected", is_sel)
                    widget.setStyleSheet(self._icon_btn_style(is_sel))
    ...
    def _on_search(self, text):
        ...
        # If empty, show all
        if not text:
            for cat, (container, grid, original_icons) in self._icon_grids.items():
                for i in range(grid.count()):
                    item = grid.itemAt(i)
                    if item and item.widget():
                        item.widget().show()
        ...
        for cat, (container, grid, original_icons) in self._icon_grids.items():
            for i in range(grid.count()):
                ...
                if item and item.widget():
                    ...
                    if icon_name:
                        matches = text in icon_name.lower()
...
class SaveToLibraryDialog(QtWidgets.QDialog):
    ...
    def _set_icon(self, icon_name):
        ...
        try:
            ...
            if icon and not icon.isNull():
                ...
                self.icon_btn.setText(icon_name[:3].upper())
                self.icon_btn.setIcon(QtGui.QIcon())
        except:
            self.icon_btn.setText(icon_name[:3].upper() if icon_name else "?")
            self.icon_btn.setIcon(QtGui.QIcon())
    ...
    def _take_screenshot(self):
        ...
        if not SNIPPING_AVAILABLE:
            ...
            return
        print("[Sopdrop] Hiding dialog for screenshot")
        self.hide()
        QtWidgets.QApplication.processEvents()
    ...
    def _check_clipboard(self):
        try:
            ...
            if clip.mimeData().hasImage():
                img = clip.image()
                if not img.isNull() and img.width() > 50:
...
class SettingsDialog(QtWidgets.QDialog):
    ...
    def _load_settings(self):
        ...
        if SOPDROP_AVAILABLE:
            from sopdrop.config import get_config, get_token, get_active_library, get_team_library_path, get_team_slug, get_team_name
    ...
    def _save_settings(self):
        ...
        if SOPDROP_AVAILABLE:
            from sopdrop.config import get_config, save_config, set_team_library_path, set_active_library, set_team_slug, set_team_name
